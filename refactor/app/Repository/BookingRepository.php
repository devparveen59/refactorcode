...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if (!empty($jobs)) {
            foreach ($jobs as $jobitem) {
                $jobList = $jobitem->immediate == 'yes' ? 'emergencyJobs' : 'normalJobs';
                ${$jobList}[] = $jobitem;
            }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => $numpages ?? null, 'pagenum' => $pagenum];
    }
    ...
    public function store($user, $data)
    {
        ...
        if (isset($data['immediate']) && $data['immediate'] == 'no') {
            $requiredFields = ['due_date', 'due_time', 'customer_phone_type'];
        ...
        // Adjust certified type if both normal and certified
        if (in_array('normal', $data['job_for']) && in_array('certified', $data['job_for'])) {
            ...
            // Assign job type based on consumer type
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        } elseif (($job->customer_physical_type == 'no' || $job->customer_physical_type == 'yes' ) && $job->customer_phone_type == 'yes') {
            // It's a phone job
    ...
    /**
     * Retrieve the current translator assigned to the job.
     *
 ...
 */
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                $mailer = new AppMailer();
                    ...
                    $email = $job->user_email ? $job->user_email : $user->email;
                $name = $user->name;
                $subject = 'Bekräftelse - tolk har accepterat er bokning (bokning # ' . $job->id . ')';
                $data = [
                    'user' => $user,
    ...
    /**
    * Send cancellation notification to translator
    * @param $translator
    * @param $job
    */

    private function sendCancellationNotificationToTranslator($translator, $job)
    {
        $data = [];
    ...
    * @param $job
    */
    private function sendCancellationNotificationToCustomer($customer, $job)
    {
        $data = [];
        $data['notification_type'] = 'job_cancelled';
        $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
 ...
 */
    ...
    /**
     * For booking expire no accpeted
     */
 ...
 */
    public function reopen($request)
    {
        ...
        if ($job['status'] != 'timedout') {
            ...
            $affectedRows = Job::create($newJobData);
     ...
     * @return \App\Models\Job The job instance.
     */
